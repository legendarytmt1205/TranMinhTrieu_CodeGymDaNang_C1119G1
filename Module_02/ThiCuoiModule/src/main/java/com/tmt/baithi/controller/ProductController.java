...
public class ProductController {
    ...
    @GetMapping("/product")
    public ModelAndView getAllCustomer(@PageableDefault(value = 20) Pageable pageable,
                                       @RequestParam(name = "name") Optional<String> name, Model model) {
        Page<Product> productList;
        pageable = PageRequest.of(pageable.getPageNumber(), 20, Sort.by("name").descending());
